...
public class RemoveCoinsCommand extends SimpleCommand {
    ...
    public RemoveCoinsCommand( String permission, String noPermissionMessage, GCoinPlugin plugin ) {
        ...
        this.plugin.getCommand( "removecoins" ).setExecutor( this::onCommand );
    }
    ...
    protected void run( CommandSender commandSender, String[] args ) {
        if ( args.length == 2 ) {
            ...
            target.sendMessage( String.format( configHandler.getMessage( "commands.remove_coins.target_message" ), coins ) );
            commandSender.sendMessage( String.format( configHandler.getMessage( "commands.remove_coins.sender_message" ), target.getName(), coins ) );

        } else {
            commandSender.sendMessage( configHandler.getWrongArgs() + "/removecoins <Player> <Amount>" );
        }
    }
